...
class Config(object):
    def __init__(self):
        ...
        self.config.read(["default_config.ini", 
            os.path.expanduser('~/Documents/openroast/config.ini')])

        # Place software version into config object
        self.config['APP'] = {'version': '1.0.0'}
    ...
    def get_screenshot_directory(self, config=None):
        config = config or self.config
        return config.get("GeneralSettings", "ScreenshotDirectory", 
            fallback = os.path.expanduser('~/Documents/openroast/Screenshots'))
    ...
    def get_roaster(self, config=None):
        config = config or self.config
        return config.get(
            "Roaster", "RoasterModel", fallback = "FreshRoastSR700")
    ...
    def write_user_config(self, config=None):
        ...
        with open(os.path.expanduser(
                '~/Documents/openroast/config.ini'), 'w') as configfile:
